...
- [x] Numerisk kantdiffraktion (Vanderkooy-stil edge-integral, on-axis): calcBaffleDiffraction med driverpositioner fra fælles baffle-layout (samme stak som CAD-eksport), Δθ-vægtede kantbidrag med faseforsinkelse → ripple + 6 dB-step i én model, roundover-dæmpning. I hele sim-stien (processBand/worker/optimizer/mål-slopes); fallback til shelf uden layout
- [x] Off-axis-diffraktion pr. vinkel: calcBaffleDiffractionOffAxis (samme edge-integral + far-field-projektion af observationsretningen på baflen) → spinorama anvender per-vinkel diffraktions-delta Δ(f,h,v) oven på ren stempeldirektivitet for positionsbevidste bånd (delta relativt til on-axis, så den indbagte on-axis-diffraktion ikke dobbelt-påføres). LW/ER/SP/DI/PIR afhænger nu af driverplaceringer + roundover; horisontal spejlsymmetri + coarse-grid-interpolation (≤72 pkt) + delta-cache holder første kald ~0,3 s og efterfølgende ~ms. Legacy-kald uden positioner uændret (effectiveBaffleDiameter). Bevidste grænser: far-field, front-halvkugle, ingen kabinetdybde/skygge; calcPolar (enkelt-driver polar) bruger fortsat kun stempelmodellen
- [x] Monteringsoverrides pr. bånd (DesignBand.mount): fast frontposition (målt x/y i mm) eller sidemontering. layoutBandPositions respekterer overrides som single source of truth → faste positioner driver kantdiffraktion, spinorama-delta og CAD-udskæring; sidemonterede enheder udelades ærligt af frontbaffel-modellen (generisk baffelstep-shelf, dokumenteret tilnærmelse — ingen kabinetdybde/side-panel-diffraktion endnu). UI på bandkortet (Auto/Fast/Side + x/y-felter)
- [x] Indbyggede presets (src/data/presets.ts + knap under Gem projekt): Kudos X2 (3-vejs) — Joachims fysiske kabinet 720×165×205 mm, port Ø70×250 (Vb ≈ 13 L / fb 54 Hz estimeret, rammer fysisk portlængde via appens egen portmodel), Vifa BC25TG15-04 y=655, Wavecor WF146WA02 y=535 (65/185 mm fra top), ScanSpeak 18W/4424G00 sidemonteret højre 360 mm fra bund, XO 300/2800 Hz LR4, diskant-pad −6 dB
- [x] Mk3 Reference-preset (6/9, fra mk3-reference-loudspeaker-repoets cabinet.scad): 2× GRS 12SW-4HE push-push sidemonteret (woofer_z 520, driverCount 2 → Vas×2), ScanSpeak 18W/4424G00 front mid_z 1065, SB26STAC-C000-4 i waveguide UNDER midten tw_z 901 (c-c 164), kabinet 300×420×1180 lukket ~75 L netto, R19 roundover, XO 200 BW4 / 1100 LR4, DSP-gains 0/−4/−9 (repoets Harman-optimering), rum 4,5×4×2,4 m RT60 0,4. Dokumenterede udeladelser: mid-kammer 13 L (appen har ét volumen), waveguide-loading +2,5 dB, Linkwitz Transform 41→28 Hz
- [x] Bruger-Vb for lukkede kabinetter: sealed-grenen i calcCabinetResponse udleder nu Fc/Qtc/F3 af alfa = Vas/Vb når Vb er sat (0 = auto Qtc 0.707) — gælder processBand (sim + worker), kabinetkortet, A/B-sammenligning og kabinetoptimizerens sealed-sweep (latent bug: sweepet ignorerede volumen). Vb-felt i UI for lukkede kabinetter
- [x] Mount-bevidst tidsjustering + forskudt frontbaffel (6/9): delt computeAutoDelays regner kun på frontmonterede bånd — sidemonterede udelades (delay røres ikke) og Tidsjusteringskortet viser dem eksplicit som udenfor tidsjustering. Pr. bånd (kun front): mekanisk plan-forskydning zMm (+ = bag hovedplanet) indgår i akustisk dybde og fasesummering (depthMm i complexSum + PhaseAlignmentCard), og egen delbaffel (B×H) giver båndet sin egen kantdiffraktionsberegning med enheden centreret på egen plade. Preset-delays forudberegnet (Mk3 0,06 ms / Kudos 0,04 ms, selvverificerende test). Trinkant mellem planer modelleres ikke; buildMiniDspConfig i cabinetMatch fortsat mount-blind (kendt begrænsning)
